    ...
    <ul>
      <li class="menu" v-for="item in tabs" :key="item.title">
        ...
        <nuxt-link class="link" v-bind:to="item.children ? '' : `/profile${item.path}`">
          <span class="menu-block" :class="item.status ? 'active-tab' : ''" @click="item.children ? openCloseTabs(item.title) : activTabsUpdate(item.path)">
        ...
        <ul class="blockSubMenu" v-if="item.children && item.status">
          <li class="subMenu" v-for="subTabs in item.children" :key="subTabs.title">
            <nuxt-link class="link" v-bind:to="`/profile${subTabs.path}`">
              <span class="subtitle" :class="subTabs.status ? 'active-tab' : ''" @click="activTabsUpdate(subTabs.path)">
                {{ subTabs.title }}
                <span class="counter" v-if="subTabs.counter">({{ subTabs.counter }})</span>
              </span>
            </nuxt-link>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    sortTabs (res) {
      ...
      res.data.data.map(tab => tab.title === 'Dashboard' ? childrenDashboard = tab.children : null);
      res.data.data.map(tab => tab.title === 'Comments' ? childrenComments = tab.children : null);
      res.data.data.map(tab => tab.title === 'Posts' ? childrenPosts = tab.children : null);

      if(childrenDashboard) {
        childrenDashboard.map(res => res['sort'] = res.title.replace(/\s+/g, ''));
        childrenDashboard.sort( (l,r) =>  dasboard[l.sort] - dasboard[r.sort] );
      }

      if(childrenComments) {
        childrenComments.map(res => res['sort'] = res.title.replace(/\s+/g, ''));
